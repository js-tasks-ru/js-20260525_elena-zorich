...
export default class ProductForm {
  ...
  private sortableList: SortableList | null = null;
  ...
  setFormData(): void {
    const productForm = this.sub('productForm');
    const excludedFields = ['images'];
    const fields = Object.keys(this.defaultFormData)
      .filter(item => !excludedFields.includes(item)) as Array<keyof ProductFormData>;
  ...
  initEventListeners(): void {
    this.sub('productForm').addEventListener('submit', this.onSubmit);
    this.sub('uploadImage').addEventListener('click', this.uploadImage);
  }
  ...
  private removeEventListeners(): void {
    this.sub('productForm').removeEventListener('submit', this.onSubmit);
    this.sub('uploadImage').removeEventListener('click', this.uploadImage);
